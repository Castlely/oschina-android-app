...
import cn.gov.psxq.bean.ListData;
...
public class HandlerManager {
     ...
     * @return
     */
    public Handler getHandler(final PullToRefreshListView lv, final BaseAdapter adapter,
                              final TextView more, final ActionBarProgressBarVisibility abProgress,
                              final LoadListDataCallbacks callbacks, final int pageSize) {
        return new Handler() {
            public void handleMessage(Message msg) {
                ...
                if (msg.arg1 == UIHelper.LISTVIEW_ACTION_REFRESH) {
                    lv.onRefreshComplete(appContext.getString(R.string.pull_to_refresh_update)
        ...
        switch (actiontype) {
            ...
            case UIHelper.LISTVIEW_ACTION_CHANGE_CATALOG:
                ...
                if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH) {
                    // 提示新加载数据
                    if (newdata > 0) {
                        NewDataToast.makeText(appContext,
                            appContext.getString(R.string.new_data_toast_message, newdata),
                            appContext.isAppSound()).show();
                    } else {
                        NewDataToast.makeText(appContext,
                            appContext.getString(R.string.new_data_toast_none), false).show();
                    }
                }
                break;
