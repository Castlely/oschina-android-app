...
public class BlogDetail extends CustomActivity implements FooterViewVisibility {
    ...
    private void initView() {
        ...
        if (blogId > 0)
            tempCommentKey = AppConfig.TEMP_COMMENT + "_" + CommentPub.CATALOG_BLOG + "_" + blogId;
    ...
    private void initCommentView() {
        ...
        mLvComment.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                // 跳转--回复评论界面
                UIHelper.showCommentReply(BlogDetail.this, curId, curCatalog, com.getId(),
        ...
        mLvComment.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (uid == com.getAuthorId()
                    || (blogDetail != null && uid == blogDetail.getAuthorId())) {
                    ...
                    final Thread thread = new Thread() {
                        public void run() {
                            Message msg = new Message();
                            try {
                                Result res = ac.delBlogComment(uid, blogId, com.getId(),
                                    com.getAuthorId(), blogDetail.getAuthorId());
                                msg.what = 1;
                                msg.obj = res;
                            } catch (AppException e) {
                                e.printStackTrace();
     ...
     *            动作标识
     */
    private void loadLvCommentData(final int id, final int pageIndex, final Handler handler,
                                   final int action) {
        new Thread() {
            public void run() {
                Message msg = new Message();
    ...
    private void share() {
        if (blogDetail == null) {
            UIHelper.ToastMessage(BlogDetail.this, R.string.msg_read_detail_fail);
            return;
        }
        // 分享到
