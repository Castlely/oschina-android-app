...
public class SearchListFragment extends Fragment implements LoadListDataCallbacks {
    ...
    public void onAttach(Activity activity) {
        ...
        try {
            ...
            throw new ClassCastException(activity.toString()
                                         + "must implenemt SearchContextListener");
        }
    }
    ...
    private void initListView(View layout, int action) {
        ...
        final View fooer = getActivity().getLayoutInflater().inflate(R.layout.listview_footer,
            mlvSearch, false);
        final TextView footMore = (TextView) fooer.findViewById(R.id.listview_foot_more);
        ListViewSearchAdapter adapter = new ListViewSearchAdapter(getActivity(), lvSearchData,
     ...
     * @param action 动作标识
     */
    private void loadLvData(final int pageIndex, final Handler handler, final int action) {
        ...
        new Thread() {
            public void run() {
                ...
                try {
                    SearchList searchList = appContext.getSearchList(curSearchCatalog,
                        searchString, pageIndex, 20);
