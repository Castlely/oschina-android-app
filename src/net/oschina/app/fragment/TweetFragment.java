...
public class TweetFragment extends ListFragment implements LoadListDataCallbacks {
     ...
     * 初始化动弹列表
     */
    protected void initListView(final int curTweetCatalog, final PullToRefreshListView lvTweet,
                                int action) {
        final ListViewTweetAdapter lvTweetAdapter = new ListViewTweetAdapter(getActivity(),
            lvTweetData, R.layout.tweet_listitem);
        final View lvTweet_footer = getActivity().getLayoutInflater().inflate(
            R.layout.listview_footer, null);
        final TextView lvTweet_foot_more = (TextView) lvTweet_footer
            .findViewById(R.id.listview_foot_more);
        HandlerManager handlerUtil = HandlerManager.getInstance();
        mHandler = handlerUtil.getHandler(lvTweet, lvTweetAdapter, lvTweet_foot_more, abProgress,
            this, AppContext.PAGE_SIZE);
        ...
        lvTweet.setOnScrollListener(new AbsListView.OnScrollListener() {
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                ...
                if (scrollEnd && lvDataState == UIHelper.LISTVIEW_DATA_MORE) {
                    ...
                    loadLvData(curTweetCatalog, pageIndex, mHandler,
        ...
        lvTweet.setOnRefreshListener(new PullToRefreshListView.OnRefreshListener() {
            public void onRefresh() {
                loadLvData(curTweetCatalog, 0, mHandler, UIHelper.LISTVIEW_ACTION_REFRESH);
            }
        });
        isInitListView = true;
    }
